...
import(
	...
	"github.com/go-worker-balance/internal/adapter/event"
	"github.com/go-worker-balance/internal/core"
	"github.com/go-worker-balance/internal/service"
...
func loadLocalEnv(){
	...
	envKafka.KafkaConfigurations.Groupid = "GROUP-01"
...
func main()  {
	...
	ctx, seg := xray.BeginSegment(context.Background(), "go-worker-balance")
	defer seg.Close(nil)
	...
	wg.Add(1)
	go consumerWorker.Consumer(ctx, &wg, "topic.x")
	wg.Wait()
}